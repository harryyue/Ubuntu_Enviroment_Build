...
class DiagramManager extends EventEmitter {
  ...
  setZoomLevel (value) {
    ...
    if (value > 3) { value = 3 }
    this.diagramEditor.canvas.zoomFactor.setScale(value)
    this.repaint()
  }
   ...
   */
  getZoomLevel () {
    return this.diagramEditor.canvas.zoomFactor.getScale()
  }
  ...
  scrollTo (x, y, animation) {
    var sz = this.getViewportSize()
    var _x = Math.max(0, Math.floor((x * this.getZoomLevel()) - (sz.x / 2)))
    var _y = Math.max(0, Math.floor((y * this.getZoomLevel()) - (sz.y / 2)))

    if (animation === false) {
      this.$diagramArea.scrollLeft(_x)
      this.$diagramArea.scrollTop(_y)
    } else {
      this.$diagramArea.animate({
        scrollLeft: _x,
        scrollTop: _y
      }, 500)
    }
      ...
      case keycode('up'):
        if (e.ctrlKey || e.metaKey) {
          if (this.diagramEditor && this.diagramEditor.diagram && this.diagramEditor.diagram.selectedViews.length > 0) {
            ...
            this._triggerViewMovedEvent(this.diagramEditor.diagram.selectedViews, 0, -dy)
            e.preventDefault()
          }
        }
        break
      ...
      case keycode('left'):
        if (e.ctrlKey || e.metaKey) {
          if (this.diagramEditor && this.diagramEditor.diagram && this.diagramEditor.diagram.selectedViews.length > 0) {
            ...
            this._triggerViewMovedEvent(this.diagramEditor.diagram.selectedViews, -dx, 0)
            e.preventDefault()
          }
        }
        break
  ...
  _setupMouseBindings () {
    $('#diagram-area').mousewheel(e => {
      if (e.ctrlKey || e.metaKey) {
        e.preventDefault()
        if (e.deltaY < 0) {
          app.commands.execute('view:zoom-out')
        } else if (e.deltaY > 0) {
          app.commands.execute('view:zoom-in')
        }
      }
    })
  }
  ...
  _setupPreferences () {
    app.preferences.on('change', (key, value) => {
      try {
        ...
        if (key === 'diagramEditor.gridSize') {
          this.diagramEditor.canvas.gridFactor.height = value
          this.diagramEditor.canvas.gridFactor.width = value
          this.repaint()
        }
      } catch (err) {
        console.error(err)
    ...
    this.diagramEditor.canvas.gridFactor.height = gridSize
    this.diagramEditor.canvas.gridFactor.width = gridSize
  }
